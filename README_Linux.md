...
คู่มือนี้เหมาะกับ Ubuntu, Debian, Fedora, Arch และ Linux distributions อื่น ๆ
...
```bash
sudo apt update
sudo apt install -y openssh-client git make rsync
```

Fedora:

```bash
sudo dnf install -y openssh-clients git make rsync
```

Arch:

```bash
sudo pacman -S openssh git make rsync
```
...
ถ้าไม่ใช้ Makefile ก็รัน command ตรงใน section ถัดไปได้
...
ถ้ามีไฟล์ใหญ่ ใช้ `rsync` จะ resume ได้ดีกว่า:
...
## 15. Troubleshooting Linux

| ปัญหา | วิธีแก้ |
|---|---|
| `ssh: Could not resolve hostname` | เช็กชื่อ host ใน `~/.ssh/config` |
| `Permission denied` | เช็ก key path, permission, และ `authorized_keys` |
| `conda activate` ไม่ได้ | `source ~/.bashrc` หรือ `source ~/miniconda3/etc/profile.d/conda.sh` |
| job pending นาน | เช็ก `squeue`, `sinfo`, partition, quota |
| CUDA ไม่เจอ | เช็กว่า script รันใน GPU job ไม่ใช่ login node |
| upload หลุด | ใช้ `rsync -avP` แทน `scp` |

---

## 16. Cheat Sheet Linux
